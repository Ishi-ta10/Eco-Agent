...
try:
    ...
    if 'inverter' in last_log:
        ...
        for inv_id, inv_data in last_log['inverter'].items():
            if isinstance(inv_data, dict):
                ...
                total_total_generation += inv_data.get('WHTot', 0)
                peak_power = max(peak_power, inv_data.get('WT', 0))
        
        # Convert to kWh
        solar_panel_data['generation_summary']['today_generation_kWh'] = round(total_day_generation / 1000, 2)
        solar_panel_data['generation_summary']['total_cumulative_generation_kWh'] = round(total_total_generation / 1000, 2)
        solar_panel_data['energy_metrics']['peak_power_kW'] = round(peak_power / 1000, 2)
            ...
            for ts_str, data in day_data.items():
                if isinstance(data, dict):
                    try:
                        ...
                        # Fallback to meter readings if inverter not available
                        if 'meter' in data and not daily_generation.get(str(reading_date)):
                            for meter_id, meter_data in data['meter'].items():
                                if isinstance(meter_data, dict) and 'WT' in meter_data:
                                    daily_generation[str(reading_date)] += meter_data.get('WT', 0) / 1000
                    except:
                        continue
    ...
    # Save to file
    with open('filtered_solar_panel_data.json', 'w') as f:
