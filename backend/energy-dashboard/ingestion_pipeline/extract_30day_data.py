...
with open('captured_api_data.json', 'r') as f:
...
try:
    ...
    print("=" * 60)
    print("EXTRACTING 30-DAY SOLAR GENERATION DATA")
    print("=" * 60)
    
    print(f"\n📅 Latest Server Date: {latest_date}")
    print(f"✓ Processing last 30 days of data...\n")
                                        ...
                                        try:
                                            if isinstance(report_entry, dict):
                                                ...
                                                if start_time > 0:
                                                    ...
                                                    daily_generation[str(reading_date)] = gen_value
    ...
    print("Determining data period (30 days → 15 days → 7 days)...")
    ...
    day_ranges = [30, 15, 7]
    dates_with_data = None
    selected_period = None
    
    for period_days in day_ranges:
        test_dates = []
        ...
        # Check if we have sufficient data for this period (at least 1 day with data)
        days_with_gen = sum(1 for d in test_dates if d['generation'] > 0)
        if days_with_gen > 0:
            dates_with_data = test_dates
            selected_period = period_days
            print(f"  ✓ Using {period_days}-day period ({days_with_gen} days with data)\n")
            break
    
    if dates_with_data is None:
        print("  ⚠️  No generation data found in any period")
        # Still create a timeline even with no data for the requested period
        dates_with_data = []
        for i in range(7):  # Default to 7 days
            check_date = latest_available_date - timedelta(days=i)
            dates_with_data.append({
                'date': str(check_date),
                'generation': daily_generation.get(str(check_date), 0),
                'days_ago': i
            })
        selected_period = 7
    ...
    if non_zero_days:
        ...
        print(f"  ✓ Total 30-day Generation: {total_gen:.2f} kWh")
        print(f"  ✓ Best Day: {peak_gen:.2f} kWh")
        print(f"  ✓ Average per Day: {avg_gen:.2f} kWh")
        print(f"  ✓ Days with Data: {len(non_zero_days)}/30\n")
    ...
    # ========== SAVE OUTPUT ==========
    output_file = 'filtered_30day_data.json'
    with open(output_file, 'w') as f:
        json.dump(monthly_data, f, indent=2)
    
    print("\n" + "=" * 60)
    print("✓ SOLAR_METER DATA EXTRACTION COMPLETE")
    print("=" * 60)
    ...
    if dates_with_data:
        ...
        print(f"  Data Source: SOLAR_METER from plantInfo (daily report)")
        print(f"  Data Granularity: ALL readings (not aggregated)")
    print(f"  Total Generation: {monthly_data['period_summary']['total_generation_30days_kWh']} kWh")
